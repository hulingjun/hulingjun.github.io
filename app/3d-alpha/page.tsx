...
export default function Alpha3D() {
  ...
  const [selected, setSelected] = useState("fact"); const [guideIndex, setGuideIndex] = useState(0); const [caseIndex, setCaseIndex] = useState(0); const [failed, setFailed] = useState(false); const [saved, setSaved] = useState(false);
  ...
  const choose = (id: string) => { setSelected(id); if (mode === "guide") setGuideIndex(Math.max(0, guided.indexOf(id))); };
  const changeMode = (next: "guide" | "free" | "case") => { setMode(next); if (next === "guide") { setGuideIndex(0); setSelected("fact"); } if (next === "case") setCaseIndex(0); };
